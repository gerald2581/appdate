    ...
    for (const c of cards) {
      if (!c.hit) {
        ...
        if (nextT >= 1 && c.laneQueue.length > 1) {
          // Card wraps to t=0 → it's off-screen at edge, safe to swap photo silently
      ...
      if (c.hit) {
        ...
        // curX/curY frozen — card stays visually in place while t advances
      } else {
        c.curS = lerp(c.curS, lerp(MAX_SCALE, MIN_SCALE, d * d), LERP_SCL)
        c.curO = lerp(c.curO, lerp(1.0, 0.38, d), LERP_SCL)
        c.rX   = lerp(c.rX, 0, LERP_ROT)
        c.rY   = lerp(c.rY, 0, LERP_ROT)
        // Smoothly slide back to circuit position on release
