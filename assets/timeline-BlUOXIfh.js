import{i as e,n as t,o as n}from"./router-SnC_tAPp.js";import{n as r,s as i}from"./index-BKN2W0tb.js";import{t as a}from"./nav-JQzAyBuE.js";import{t as o}from"./escape-1QVFIIou.js";import{n as s}from"./storage-j0tHUYzK.js";import{t as c}from"./confirm-modal-CDDWdlXn.js";var l=82,u=82,d=1.2,f=.72,p=.024,m=.048,h=.08,g=39683,_=160,v=84,y=7,b=[`200,130,106`,`122,158,200`,`106,184,122`,`212,149,106`,`160,130,200`,`180,160,106`];function x(e,{p0:t,p1:n,p2:r}){let i=1-e;return{x:i*i*t.x+2*i*e*n.x+e*e*r.x,y:i*i*t.y+2*i*e*n.y+e*e*r.y}}function S(e,t){let n=(_+t-v)/2,r=u*.55,i=_+24,a=t-v-24;return[{p0:{x:-l,y:n-r},p1:{x:e*.5,y:i},p2:{x:e+l,y:n-r}},{p0:{x:-l,y:n+r},p1:{x:e*.5,y:a},p2:{x:e+l,y:n+r}}]}function C(e,t,n){return e+(t-e)*n}function w(e,t){if(e.inner.innerHTML=``,t.photoUrl){let n=document.createElement(`img`);n.src=t.photoUrl,n.draggable=!1,n.style.cssText=`
  ...
  `,j.appendChild(M),document.body.appendChild(j);let N={memory:`Kenangan`,milestone:`Milestone`,photo:`Foto`},P=()=>{j.dataset.mode=``,j.style.pointerEvents=`none`,j.style.background=`rgba(0,0,0,0)`,j.style.backdropFilter=``,j.style.alignItems=`flex-end`,M.style.transform=`translateY(100%)`,M.style.maxHeight=`82vh`,M.style.maxWidth=`430px`,M.style.borderRadius=`28px 28px 0 0`,M.style.borderTop=`1px solid rgba(255,255,255,0.85)`,M.style.background=`rgba(255,255,255,0.97)`,M.style.boxShadow=`0 -8px 40px rgba(0,0,0,0.15)`,M.style.backdropFilter=`blur(32px)`,M.style.display=`block`,M.style.width=`100%`,M.style.height=`auto`,M.style.overflowY=`auto`,M.style.flexDirection=``,M.style.alignItems=``,M.style.justifyContent=``,M.style.padding=``},F=(e,t)=>{if(j.style.pointerEvents=`all`,e.photoUrl){j.dataset.mode=`lightbox`,j.style.background=`rgba(253,244,240,0.90)`,j.style.backdropFilter=`blur(24px) saturate(1.3)`,j.style.alignItems=`stretch`,M.style.transform=`none`,M.style.maxHeight=`100vh`,M.style.borderRadius=`0`,M.style.background=`transparent`,M.style.boxShadow=`none`,M.style.backdropFilter=`none`,M.style.borderTop=`none`,M.style.display=`flex`,M.style.flexDirection=`column`,M.style.alignItems=`center`,M.style.justifyContent=`flex-start`,M.style.width=`100%`,M.style.maxWidth=`100%`,M.style.height=`100%`,M.style.padding=`0`,M.style.overflowY=`hidden`;let a=e=>`
    ...
    `;let v=document.createElement(`div`);v.style.cssText=`
      ...
      padding:3px;
    `,_.appendChild(v),A.appendChild(_);let y=o.x-l/2-l*(c-1)/2,S=o.y-u/2-u*(c-1)/2;_.style.transform=`translate(${y}px,${S}px) scale(${c})`,_.style.opacity=String(p);let T={el:_,inner:v,lane:r,t:a,phase:i,phaseSlot:t,lastCycle:Math.floor(Date.now()/g),curS:c,curO:p,curX:o.x,curY:o.y,rX:0,rY:0,tRX:0,tRY:0,hit:!1,rgb:b[t%b.length],queue:e,qIdx:h,get mem(){return this.queue[this.qIdx]}};w(T,e[h]);let E=(e,t)=>{T.hit=!0;let n=_.getBoundingClientRect(),r=(e-(n.left+n.width/2))/(n.width/2);T.tRX=-((t-(n.top+n.height/2))/(n.height/2))*22,T.tRY=r*22},D=()=>{T.hit=!1,T.tRX=0,T.tRY=0};return _.addEventListener(`mousemove`,e=>E(e.clientX,e.clientY)),_.addEventListener(`mouseleave`,D),_.addEventListener(`touchmove`,e=>{e.preventDefault(),E(e.touches[0].clientX,e.touches[0].clientY)},{passive:!1}),_.addEventListener(`touchend`,D),_.addEventListener(`click`,()=>F(T.mem,_)),T};for(let e=0;e<2;e++){let t=z[e],n=Math.min(y,t.length);for(let r=0;r<n;r++)B.push(V(t,r,n,e))}let H=0,U=()=>{if(!_.isConnected){cancelAnimationFrame(H),j.remove();return}let e=Math.min(window.innerWidth,430),t=window.innerHeight;R=S(e,t);for(let e of B){let t=(Date.now()/g+e.phase)%1;if(e.t>.85&&t<.15){let n=x(t,R[e.lane]);e.curX=n.x,e.curY=n.y}e.t=t;let n=Math.floor(Date.now()/g);n!==e.lastCycle&&(e.lastCycle=n,e.queue.length>1&&(e.qIdx=(e.phaseSlot+n)%e.queue.length,w(e,e.queue[e.qIdx])));let r=x(e.t,R[e.lane]),i=Math.abs(e.t-.5)*2;e.hit?(e.curS=C(e.curS,d+.15,p),e.curO=1,e.rX=C(e.rX,e.tRX,h),e.rY=C(e.rY,e.tRY,h)):(e.curS=C(e.curS,C(d,f,i*i),p),e.curO=C(e.curO,C(1,.55,i),p),e.rX=C(e.rX,0,h),e.rY=C(e.rY,0,h),e.curX=C(e.curX,r.x,m),e.curY=C(e.curY,r.y,m)),e.el.style.zIndex=e.hit?`9999`:String(Math.round(e.curS*300));let a=e.curX-l/2-l*(e.curS-1)/2,o=e.curY-u/2-u*(e.curS-1)/2;e.el.style.transform=e.hit?`translate(${a}px,${o}px) perspective(900px) rotateX(${e.rX}deg) rotateY(${e.rY}deg) scale(${e.curS})`:`translate(${a}px,${o}px) scale(${e.curS})`,e.el.style.opacity=String(e.curO),e.inner.style.boxShadow=e.hit?`0 8px 24px rgba(${e.rgb},.38), 0 0 14px rgba(${e.rgb},.18), inset 0 1px 0 rgba(255,255,255,.95)`:`inset 0 1px 0 rgba(255,255,255,.9)`}H=requestAnimationFrame(U)};return H=requestAnimationFrame(U),k.querySelector(`#btn-add`).addEventListener(`click`,()=>t(`/timeline/add`)),_.appendChild(a()),_}export{T as renderTimeline};